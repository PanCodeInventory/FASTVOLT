...
def generate_pdf_report(metadata: FCSMetadata) -> bytes:
    ...
    if metadata.panel_table and metadata.panel_table.rows:
        ...
        # Header row 2: Sample | label1 | label2 | ... | ""
        h2 = ["Sample"]
        ...
        # Data rows - strip newlines to avoid black blocks in PDF
        for row in pt.rows:
            clean_row = []
            for cell in row:
                cell_str = str(cell) if cell is not None else ''
                # Replace newlines (from Word/Excel paste) with spaces
                cell_str = cell_str.replace('\r\n', ' ').replace('\r', ' ').replace('\n', ' ')
                clean_row.append(cell_str)
            table_data.append(clean_row)
        
        # Column widths: Sample, each FL, Comments
        total_col_width = 19 * cm
        fl_fixed = 3.0 * cm + 2.0 * cm  # Sample + Comments
        fl_available = total_col_width - fl_fixed
        fl_each = max(1.2*cm, fl_available / max(n_fl_cols, 1))
        col_widths = [3.0*cm] + [fl_each] * n_fl_cols + [2.0*cm]
    ...
    footer_text = "Generated by FASTVOLT, Developed by PCS"
